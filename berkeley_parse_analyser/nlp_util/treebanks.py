...
def remove_trivial_unaries(tree, in_place=True):
    ...
    if in_place:
        ...
        tree = pstree.PSTree(tree.word, tree.label, tree.span, None, subtrees)
...
def remove_traces(tree, in_place=True):
    ...
    '''
    return remove_nodes(tree, pstree.PSTree.is_trace, in_place)
...
def remove_function_tags(tree, in_place=True):
    ...
    if in_place:
        ...
        tree = pstree.PSTree(tree.word, label, tree.span, None, subtrees)
        for subtree in subtrees:
            subtree.parent = tree
    return tree
...
def homogenise_tree(tree, tag_set=ptb_tag_set):
    ...
    if tree.label != 'ROOT':
        ...
        if split_label_type_and_function(tree.label)[0] not in tag_set:
            ...
            root = pstree.PSTree(None, 'ROOT', tree.span, None, [])
...
def ptb_read_tree(source, return_empty=False, allow_empty_labels=False, allow_empty_words=False, blank_line_coverage=False):
    ...
    tree = pstree.tree_from_text(cur_text, allow_empty_labels, allow_empty_words)
    ptb_cleaning(tree)
    return tree
